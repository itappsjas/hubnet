...
const flightData: { [key: string]: FlightData[] } = {
  QR: [
    ...
    // SUB Station
    {
      flightNumber: "QR-8835",
      route: "DOH-SUB",
      station: "SUB",
      cargo: { weight: "2.8T", pieces: "2,800Pcs", volume: "2.8m³" },
      pax: { capacity: 350, booked: 285, loadFactor: "81%" },
      status: "sent",
      etd: "15:20",
      eta: "03:45+1",
    },
    {
      flightNumber: "QR-8837",
      route: "DOH-SUB",
      station: "SUB",
      cargo: { weight: "3.4T", pieces: "3,400Pcs", volume: "3.4m³" },
      pax: { capacity: 350, booked: 318, loadFactor: "91%" },
      status: "sent",
      etd: "19:30",
      eta: "07:15+1",
    },
    {
      flightNumber: "QR-8839",
      route: "DOH-SUB",
      station: "SUB",
    ...
    // DPS Station
    {
      flightNumber: "QR-8841",
      route: "DOH-DPS",
      station: "DPS",
      cargo: { weight: "3.1T", pieces: "3,100Pcs", volume: "3.1m³" },
      pax: { capacity: 350, booked: 295, loadFactor: "84%" },
      status: "sent",
      etd: "16:15",
      eta: "04:30+1",
    },
    {
      flightNumber: "QR-8843",
      route: "DOH-DPS",
      station: "DPS",
      cargo: { weight: "2.6T", pieces: "2,600Pcs", volume: "2.6m³" },
      pax: { capacity: 350, booked: 305, loadFactor: "87%" },
      status: "sent",
      etd: "13:45",
      eta: "02:00+1",
    },
    {
      flightNumber: "QR-8845",
      route: "DOH-DPS",
      station: "DPS",
      cargo: { weight: "3.7T", pieces: "3,700Pcs", volume: "3.7m³" },
      pax: { capacity: 350, booked: 328, loadFactor: "94%" },
      status: "sent",
      etd: "21:30",
      eta: "09:45+1",
    },
...
function AirlineFlightsContent() {
  ...
  const { isAuthorized, isLoading, user } = useAuthCheck(['admin', 'view', 'airline']);
  ...
  // Pagination and Search State (must be before any conditional return)
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState("");
  const [itemsPerPage, setItemsPerPage] = useState(5);
  const [selectedStation, setSelectedStation] = useState("CGK");

  // Filter flights based on station and search term
  const filteredFlights = useMemo(() => {
    let filtered = flights.filter(
      (flight: FlightData) => flight.station === selectedStation
    );
    ...
    return filtered;
  }, [flights, selectedStation, searchTerm]);
                  ...
                  <div className="bg-blue-500/10 border border-blue-400/20 rounded-lg px-3 py-2 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {user?.role.toLowerCase() === 'airline' ? (
                          <select
                            value={selectedStation}
                            onChange={(e) => handleStationChange(e.target.value)}
                            className="bg-blue-900/30 border border-blue-400/30 rounded px-2 py-1 text-sm font-bold text-white focus:outline-none focus:border-blue-400/50 transition-all duration-200 cursor-pointer"
                          >
                            <option value="CGK" className="bg-slate-800">Jakarta (CGK)</option>
                            <option value="SUB" className="bg-slate-800">Surabaya (SUB)</option>
                            <option value="DPS" className="bg-slate-800">Denpasar (DPS)</option>
                          </select>
                        ) : (
                          <p className="text-sm font-bold text-white">
                            {selectedStation === 'CGK' && 'Jakarta (CGK)'}
                            {selectedStation === 'SUB' && 'Surabaya (SUB)'}
                            {selectedStation === 'DPS' && 'Denpasar (DPS)'}
                          </p>
                        )}
                      </div>
                    </div>
